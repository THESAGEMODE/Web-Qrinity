document.addEventListener("DOMContentLoaded", () => {
  ...
  const dashboardData = {
    Today: {
      sales: "Rp 8.420.000",
      salesTrend: "↑ +4.2% vs yesterday",
      orders: "124",
      ordersTrend: "↑ +6.8% vs yesterday",
      avg: "Rp 67.900",
      avgTrend: "↑ +1.2% vs yesterday",
      cust: "18",
      custTrend: "↑ +12.5% vs yesterday",
      chartPoints:
        "40,120 80,110 120,95 160,80 200,60 240,50 280,40 320,35 360,25 400,20",
      barHeights: [60, 45, 90, 110, 80, 120, 130],
    },
    Yesterday: {
      sales: "Rp 7.890.000",
      salesTrend: "↓ -2.1% vs day before",
      orders: "115",
      ordersTrend: "↓ -1.5% vs day before",
      avg: "Rp 68.600",
      avgTrend: "↑ +0.5% vs day before",
      cust: "15",
      custTrend: "↓ -5.0% vs day before",
      chartPoints:
        "40,140 80,125 120,130 160,110 200,105 240,90 280,85 320,70 360,65 400,60",
    ...
    "This Week": {
      ...
      orders: "845",
      ordersTrend: "↑ +8.2% vs last week",
      avg: "Rp 68.850",
      avgTrend: "↓ -1.5% vs last week",
      cust: "128",
      custTrend: "↑ +20.4% vs last week",
      chartPoints:
        "40,130 80,90 120,110 160,70 200,75 240,45 280,35 320,40 360,30 400,28",
      barHeights: [115, 100, 105, 110, 98, 90, 125],
    },
    "Last 30 Days": {
      sales: "Rp 245.890.000",
      salesTrend: "↑ +18.4% vs prev 30 days",
      orders: "3.580",
      ordersTrend: "↑ +14.2% vs prev 30 days",
      avg: "Rp 68.680",
      avgTrend: "↑ +2.4% vs prev 30 days",
      cust: "542",
      custTrend: "↑ +25.1% vs prev 30 days",
      chartPoints:
        "40,80 80,70 120,60 160,50 200,45 240,35 280,30 320,25 360,20 400,15",
      barHeights: [130, 110, 125, 115, 120, 135, 140],
    },
    "This Month": {
      sales: "Rp 189.500.000",
      salesTrend: "↑ +15.2% vs last month",
      orders: "2.760",
      ordersTrend: "↑ +11.8% vs last month",
      avg: "Rp 68.650",
      avgTrend: "↑ +1.8% vs last month",
      cust: "410",
      custTrend: "↑ +22.5% vs last month",
      chartPoints:
        "40,100 80,90 120,80 160,75 200,65 240,55 280,45 320,40 360,30 400,22",
  ...
  const salesOverviewData = {
    "This Week":
      "40,130 80,90 120,110 160,70 200,75 240,45 280,35 320,40 360,30 400,28",
    "Last Week":
      "40,140 80,120 120,115 160,95 200,90 240,80 280,75 320,85 360,60 400,50",
    "This Month":
      "40,100 80,90 120,80 160,75 200,65 240,55 280,45 320,40 360,30 400,22",
  };
  ...
  function updateDashboardStats(data) {
    ...
    if (salesTrend) {
      ...
      salesTrend.className =
        "stat-trend " +
        (data.salesTrend.includes("↓") ? "trend-down" : "trend-up");
    }
    ...
    if (ordersTrend) {
      ...
      ordersTrend.className =
        "stat-trend " +
        (data.ordersTrend.includes("↓") ? "trend-down" : "trend-up");
    }
    ...
    if (avgTrend) {
      ...
      avgTrend.className =
        "stat-trend " +
        (data.avgTrend.includes("↓") ? "trend-down" : "trend-up");
    }
    ...
    if (custTrend) {
      ...
      custTrend.className =
        "stat-trend " +
        (data.custTrend.includes("↓") ? "trend-down" : "trend-up");
    }
  }
  ...
  function updateLineChart(pointsString) {
    ...
    const ys7 = POINTS_X.map(
      (_, i) => newYs[Math.min(i, newYs.length - 1)] || NEW_BOTTOM,
    );
    ...
    const areaPath =
      linePath + ` L${lastX},${NEW_BOTTOM} L${POINTS_X[0]},${NEW_BOTTOM} Z`;

    const lineEl = document.getElementById("sales-line-path");
    const areaEl = document.querySelector(
      "#sales-overview-svg path:first-of-type",
    );
  ...
  function initCustomSelectors() {
    ...
    function setupDropdown(element, options, currentDefault, onSelectCallback) {
      ...
      element.addEventListener("click", (e) => {
        ...
        document
          .querySelectorAll(".date-filter-box, .mini-selector")
          .forEach((box) => {
            ...
            if (opt === "Custom Range...") {
              ...
              showCustomDatePickerModal((startDate, endDate) => {
                ...
                const customData = {
                  ...
                  cust: "32",
                  chartPoints:
                    "40,110 80,95 120,80 160,90 200,70 240,65 280,55 320,50 360,40 400,32",
    ...
    miniSelectors.forEach((selector) => {
      ...
      if (isWhite) {
        ...
        const headingText =
          cardTitle && cardTitle.querySelector("h3")
            ? cardTitle.querySelector("h3").textContent
            : "";
    ...
    document.addEventListener("click", (e) => {
      if (
        !e.target.closest(".date-filter-box") &&
        !e.target.closest(".mini-selector")
      ) {
        document
          .querySelectorAll(".date-filter-box, .mini-selector")
          .forEach((box) => {
  ...
  const NOTIFICATIONS = [
    {
      ...
      title: "New Order",
      desc: "Table 5 has placed a new order.",
      time: "2 minutes ago",
      read: false,
      link: "orders.html",
    },
    {
      ...
      desc: "QRIS payment from Table 3 has been verified.",
      time: "5 minutes ago",
      read: false,
      link: "orders.html",
    },
    {
      ...
      title: "Order Ready",
      desc: "Kitchen has finished Order #128.",
      time: "10 minutes ago",
      read: false,
      link: "orders.html",
    },
    {
      id: 4,
      type: "customer-calling",
      icon: "🔔",
      iconClass: "notif-icon-blue",
      title: "Customer Calling",
      desc: "Table 8 is requesting assistance.",
      time: "15 minutes ago",
      read: false,
      link: "tables.html",
    },
    {
      ...
      title: "Delayed Order",
      desc: "Order #115 has exceeded preparation time.",
      time: "18 minutes ago",
      read: false,
      link: "orders.html",
    },
    {
      ...
      type: "daily-report",
      icon: "📊",
      iconClass: "notif-icon-teal",
      title: "Daily Report Ready",
      desc: "Today's analytics report has been generated.",
      time: "1 hour ago",
      read: false,
      link: "analytics.html",
    },
    {
      ...
      desc: "Manual payment from Table 6 could not be processed.",
      time: "1 hour ago",
      read: true,
      link: "orders.html",
    },
    {
      ...
      desc: "A reservation for Table 10 at 7:00 PM has been made.",
      time: "2 hours ago",
      read: true,
      link: "tables.html",
    },
    {
      ...
      iconClass: "notif-icon-purple",
      title: "System Update",
      desc: "Qrinty has been updated to v2.4.1 with new features.",
      time: "3 hours ago",
      read: true,
      link: "#",
    },
  ...
  // Create mobile sidebar overlay backdrop
  let sidebarOverlay = null;
  const dbContainer = document.querySelector(".db-container");
  if (dbContainer) {
    sidebarOverlay = document.createElement("div");
    sidebarOverlay.className = "sidebar-overlay";
    dbContainer.appendChild(sidebarOverlay);
  }

  const setOverlayVisible = (visible) => {
    if (!sidebarOverlay) return;
    sidebarOverlay.classList.toggle("is-visible", visible);
  };
  ...
  const syncSidebarBrandState = () => {
    ...
    const canExpand =
      isDesktopSidebar() && sidebar.classList.contains("closed");
    ...
    if (closeSidebar) {
      closeSidebar.addEventListener("click", () => {
        if (isDesktopSidebar()) {
          ...
          setSidebarState({ mobileOpen: false });
        }
      });
    }
